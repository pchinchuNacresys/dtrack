...
#         data = request.data
        ...
        # for row in result:
        #     sender_gln = row['SenderGLN']
        #     receiver_gln = row['ReceiverGLN']
        ...
        #                 r.execute("EXEC update_receiving_guid_map @identifier = %s", [instance_id])

        #             with connection.cursor() as s:
        #                 s.execute("EXEC usp_DumpTempReceiveScanDetails @identifier = %s", [instance_id])

        #             with connection.cursor() as t:
        #                 t.execute("EXEC clear_rec_temp_data @identifier = %s", [instance_id])
        ...
        #                     u.execute("EXEC update_receiving_sscc_event @identifier = %s", [instance_id])
        ...
        #                     v.execute("EXEC update_receiving_sscc_event_error @identifier = %s", [instance_id])
...
#         for row in result:
#             instance_id=row['UUID']
#             print("UUID",instance_id,type(instance_id))
...
#                         # Execute DB procedures inside context managers
#                         with connection.cursor() as r:
#                             r.execute("EXEC update_comm_sscc_guid_map @identifier = %s", [instance_id])

#                         print("-------- here 1 ---------")

#                         with connection.cursor() as s:
#                             s.execute("EXEC usp_DumpTempCommssccScanDetails @identifier = %s", [instance_id])
                        
#                         print("-------- here 2 ---------")

#                         with connection.cursor() as t:
#                             t.execute("EXEC clear_comm_sscc_temp_data @identifier = %s", [instance_id])
...
#                             print("========message success=======")
#                             with connection.cursor() as u:
#                                 u.execute("EXEC update_comm_sscc_event @identifier = %s", [instance_id])
#                                 print("=====status updated 1=====")
#                             return Response({"log": status_response.get("logList", [])})
...
#                             print("========technical error=======")
#                             with connection.cursor() as v:
#                                 v.execute("EXEC update_comm_sscc_event_error @identifier = %s", [instance_id])
#                                 print("=====status updated 2=====")
#                             return Response({"log": status_response.get("logList", [])})
...
#                             # Optional: Log this for further analysis
#                             with connection.cursor() as w:
#                                 w.execute("EXEC update_comm_sscc_event_unknown @identifier = %s", [instance_id])
#                             return Response({
#                                 "error": f"Unhandled message status: {message_status}",
#                                 "log": status_response.get("logList", [])
#                             }, status=400)
...
#                         # Optional: Log this for further analysis
#                         with connection.cursor() as w:
#                             w.execute("EXEC update_stolen_sscc_event_unknown @identifier = %s", [instance_id])
#                         return Response({
#                             "error": f"Unhandled message status: {message_status}",
#                             "log": status_response.get("logList", [])
#                         })
...
#             # Fetch stored record using second procedure
#             cursor.execute("EXEC fetch_commscan_dataone @identifier = %s, @barcode = %s", [identifier, sscc])
#             columns = [col[0] for col in cursor.description]
#             results = [dict(zip(columns, row)) for row in cursor.fetchall()]
...
#             shipment_permit=row['shipment_permit']
...
#                             <EPCISBody>
#                                 <EventList>
#                                     <!--Commissioning of SGTIN's/PRODUCTS-->
#                                     <ObjectEvent>
#                                         <eventTime>{event_time_str}</eventTime>
#                                         <eventTimeZoneOffset>{event_time_zone_offset}</eventTimeZoneOffset>
...
#                         <epcis:EPCISDocument 
#                             schemaVersion="1.2" 
#                             creationDate="2022-01-10T18:51:26Z" 
...
#                                         <sbdh:Standard>EPCGlobal</sbdh:Standard>
#                                         <sbdh:TypeVersion>1.0</sbdh:TypeVersion>
#                                         <!-- Mandatory. Instance Identifier up to 50 char (min 32) alphanumeric UUID -->
#                                         <sbdh:InstanceIdentifier>{instance_id}</sbdh:InstanceIdentifier>
#                                         <sbdh:Type>Events</sbdh:Type>
#                                         <sbdh:CreationDateAndTime>{event_time_str}</sbdh:CreationDateAndTime>
#                                     </sbdh:DocumentIdentification>
...
#                                 <EventList>
#                                     <ObjectEvent>
#                                         <!-- COMMISSION EVENT FOR SHIPPER CASES -->
#                                         <eventTime>{event_time_str}</eventTime>
#                                         <eventTimeZoneOffset>{event_time_zone_offset}</eventTimeZoneOffset>
#                                         <epcList><epc>{sgtin}</epc></epcList>
#                                         <action>ADD</action>
...
#                 if response.status_code == 202:
#                     # Successful post to Tatmeen API
#                     with connection.cursor() as r:
#                         r.execute("EXEC update_lost_guid_map @identifier = %s", [instance_id])
#                     with connection.cursor() as s:
#                         s.execute("EXEC usp_DumpTempLostScanDetails @identifier = %s", [instance_id])
...
#                     status_response = get_tatmeen_instance_status(instance_id)
#                     message_status = status_response.get("messagestatus", "").strip()
#                     log_list = status_response.get("logList", [])
...
#                 else:
#                     # API did not accept the payload, schedule retry
#                     send_lost_sscc_to_tatmeen.send(xml_body, token_data['access_token'])
#                     processing_results.append({
#                         "instance_id": instance_id,
#                         "status": "retry_scheduled",
#                         "http_status": response.status_code,
#                         "reason": "Tatmeen API did not return 202"
#                     })
...
#         return Response({
#             "message": "Lost SSCC processing completed.",
...
#                     # DB updates based on message status
#                     if message_status == "S - Successful":
#                         with connection.cursor() as u:
#                             u.execute("EXEC update_sample_guid_map @identifier = %s", [instance_id])
#                         with connection.cursor() as s:
#                             s.execute("EXEC usp_DumpTempSampleScanDetails @identifier = %s", [instance_id])
#                         with connection.cursor() as t:
#                             t.execute("EXEC clear_sample_temp_data @identifier = %s", [instance_id])
...
#                 else:
#                     # Retry logic - custom signal handler
#                     send_sample_sscc_to_tatmeen.send(xml_body, token_data['access_token'])
#                     processing_results.append({
#                         "instance_id": instance_id,
#                         "status": "retry_scheduled",
...
class ReturnReceivingAPIView(APIView):
    def post(self, request):
        ...
        #             if message_status == "S - Successful":
        #                 with connection.cursor() as u:
        #                     u.execute("EXEC update_ret_rec_guid_map @identifier = %s", [instance_id])
        #                 with connection.cursor() as s:
        #                     s.execute("EXEC usp_DumpTempRetRecScanDetails @identifier = %s", [instance_id])
        #                 with connection.cursor() as t:
        #                     t.execute("EXEC clear_ret_rec_temp_data @identifier = %s", [instance_id])
...
#     def post(self, request, *args, **kwargs):
#         parent_barcode = request.data.get('barcode', '')
#         child_barcode = request.data.get('child_barcode', '')
#         identifier = request.data.get('identifier', '')
...
#         def parse_sgtin(raw_data):
#             raw_data = raw_data.lstrip(chr(29) + '>')
#             idx = raw_data.find('01')
#             if idx == -1:
#                 raise ValueError("AI '01' not found in barcode")
#             raw_data = raw_data[idx:]
#             pattern = r"01(\d{14})17(\d{6})10(.+)"
#             match = re.match(pattern, raw_data)
#             if not match:
#                 raise ValueError("Invalid SGTIN format")
#             return match.groups()  # gtin, expiration_date, batch_number

#         try:
#             parent_gtin, _, _ = parse_sgtin(parent_barcode)
#             child_gtin, _, _ = parse_sgtin(child_barcode)

#             sgln_urn = generate_sgln_urn(sender_gln)
#             print("====SGLN URN:", sgln_urn)

#             parent_sgtin = generate_sgtin_from_gtin(parent_gtin, ProductMaster, vendor_master)
#             print("====Parent SGTIN:", parent_sgtin)

#             child_sgtin = generate_sgtin_from_gtin(child_gtin, ProductMaster, vendor_master)
#             print("====Child SGTIN:", child_sgtin)
...
#             # Fetch related EPC data after inserts (optional, for UI)
#             cursor.execute("EXEC DisAggEAfromBEA_data_one @identifier = %s, @barcode = %s", [identifier, parent_gtin])
#             columns = [col[0] for col in cursor.description]
#             results = [dict(zip(columns, row)) for row in cursor.fetchall()]
...
#     def post(self, request, *args, **kwargs):
#         parent_barcode = request.data.get('barcode', '')#parent barcode
#         child_barcode = request.data.get('child_barcode', '')#child barcode
#         identifier = request.data.get('identifier', '')
...
#             # Fetch related EPC data after inserts (optional, for UI)
#             cursor.execute("EXEC AggEachesIntoCase_data_one @identifier = %s, @barcode = %s", [identifier, parent_gtin])
#             columns = [col[0] for col in cursor.description]
#             results = [dict(zip(columns, row)) for row in cursor.fetchall()]
...
#     Rest of the data (identifier, sender_gln, receiver_gln, supplier_gln, sgln_urn)
...
#         required_columns = ["epc", "eventTime", "parent", "permit", "expiryDate", "manufDate"]
...
#                     sgtin = self.generate_sgtin_from_gtin_and_serial(gtin, serial, ProductMaster, VendorMaster)
