...
#             username = data['username']
#             cursor = connections['default'].cursor()
#             cursor.execute("EXEC fetch_user_details_profile_info'" + str(username) + "'")
#             columns = [col[0] for col in cursor.description]
#             profile_info = [dict(zip(columns, row)) for row in cursor.fetchall()]
#             return Response({'sprofile_info_data': profile_info}, status=status.HTTP_200_OK)
#         except Exception as e:
#             return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
#         extension_digit = request.data.get("extension_digit")
#         company_prefix = request.data.get("company_prefix")
#         serial_reference = request.data.get("serial_reference")

#         if not (extension_digit.isdigit() and len(extension_digit) == 1):
#             return Response({"error": "Invalid extension digit"}, status=status.HTTP_400_BAD_REQUEST)
#         if not (company_prefix and company_prefix.isdigit()):
#             return Response({"error": "Invalid company prefix"}, status=status.HTTP_400_BAD_REQUEST)
#         if not (serial_reference and serial_reference.isdigit()):
#             return Response({"error": "Invalid serial reference"}, status=status.HTTP_400_BAD_REQUEST)

#         base_sscc = extension_digit + company_prefix + serial_reference
#         if len(base_sscc) != 17:
#             return Response({"error": "Base SSCC must be 17 digits"}, status=status.HTTP_400_BAD_REQUEST)
...
#         # ean = barcode.get_barcode_class('code128')
#         # buffer = io.BytesIO()
#         # ean(gs1_data, writer=ImageWriter()).write(buffer)
#         # buffer.seek(0)
        
#         # gs1_data = f"{full_sscc}"  # Only the 18-digit number, no parentheses
#         # display_text = f"00{full_sscc}"  # For human-readable text only
...
#         # Create or retrieve database entry
#         barcode_record, created = SSCCBarcode.objects.get_or_create(
#             full_sscc=full_sscc,
#             defaults={
#                 'extension_digit': extension_digit,
...
#         # If already exists but image not stored, save it
#         if not created and not barcode_record.barcode_image:
#             barcode_record.barcode_image.save(filename, content_file, save=True)
...
#                         @area=%s
#                 """, [
#                     company_name, account_alias, gln, company_prefix, email, contact_no,
#                     location_name, location_type, postal_code, street,
#                     country_code, city, region, gps_coordinates, area
#                 ])
...
#                 'company_name', 'GLN', 'company_prefix', 'ContactNo',
#                 'LocationType', 'LocationName', 'account_alias',
#                 'email', 'postal_code', 'street', 'country_code',
#                 'city', 'region', 'gps_coordinates', 'area','status'
#             )
...
#                 cleaned = {
#                     key: (
#                         "Active" if key == "status" and company[key] == 1 else
#                         # "Inactive" if key == "status" and company[key] == 0 else
#                         company[key] if company[key] not in [None, ""] else "N/A"
#                     )
#                     for key in company
#                 }
